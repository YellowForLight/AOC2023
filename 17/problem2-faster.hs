...
findPath h s p 
    | rs == [] && ds == [] && forward >= 4 = (h, heat)
    | rs == [] && ds == [] = findPath h s next
    | forward == 10 = uncurry (findPath h) (foldr combine (s, next) (mapMaybe (inner 1) (adjacent d0)))
    | forward < 4 && not (forward == 0) = uncurry (findPath h) (foldr combine (s, next) (mapMaybe (inner (forward+1)) [d0]))
