...
findPath close s p 
    | rs == [] && ds == [] && forward >= 4 = return (0, heat)
    | rs == [] && ds == [] = findPath close s next
    | getH z < close = (getH z, heat) : findPath (getH z) s p
    | forward == 10 = uncurry (findPath close) (foldr combine (s, next) (mapMaybe (inner 1) (adjacent d0)))
    | forward < 4 && not (forward == 0) = uncurry (findPath close) (foldr combine (s, next) (mapMaybe (inner (forward+1)) [d0]))
    | otherwise = uncurry (findPath close) (foldr combine (s, next) (mapMaybe (inner (forward+1)) [d0] ++ mapMaybe (inner 1) (adjacent d0)))
    where
        ...
        other f d z2 
            ...
            | otherwise = Just ((d, f, position z2), (heat + getC z2 + getH z2, (heat + getC z2, f, d, z2)))

calc :: Integral a => Read a => String -> [(a, a)]
calc = (findPath 10000000000 S.empty) . P.fromList . pure . (0,) . (0, 0, Right,) . fmap (fmap (read . pure)) . fromJust . zippify . lines
