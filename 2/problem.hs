...
ids :: Integral a => Read a => String -> (a, Bool)
ids  = bimap getId (all id . getZipList . (pure (>=) <*> ZipList [12, 13, 14] <*>) . parse . drop 1) . break (==':')
    where
        getId :: Read a => Integral a => String -> a
        getId (stripPrefix "Game " -> Just s) = read s

calc :: Integral a => Read a => String -> a
calc = sum . fmap fst . filter snd . fmap ids . lines
