...
rights :: Zipperr a -> [Zipperr a]
rights = fmap fromJust . takeWhile isJust . iterate (>>= right) . return
rightMost :: Zipperr a -> Zipperr a
rightMost = last . rights
downs :: Zipperr a -> [Zipperr a]
downs = fmap fromJust . takeWhile isJust . iterate (>>= down) . return
downMost :: Zipperr a -> Zipperr a
downMost = last . downs
...
move :: Integral a => Set ((a, a), Direction) -> [(Direction, Maybe (Zipperr Char))] -> Set (a, a)
move v [] = S.empty
...
move' :: Integral a => Direction -> Zipperr Char -> Set (a, a)
move' d z = move S.empty [(d, pure z)]

moveAll :: Integral a => Zipperr Char -> [Set (a, a)]
moveAll z = fmap (move' Down) (rights z) ++ fmap (move' Right) (downs z) ++ fmap (move' Up) (rights $ downMost z) ++ fmap (move' Left) (downs $ rightMost z)

calc :: String -> [Int]
calc = fmap S.size . moveAll . fromJust . zippify . lines
