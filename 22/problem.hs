...
data Block a = Block {getX :: a, getY :: a, getZ :: a, dir :: Direction, size :: a} deriving (Eq, Ord, Show, Read)
...
dropBlocks b (m1, m2) 
    ...
    | any ((==1) . thrd) (getRange b) = (foldr (flip M.insert b) m1 (getRange b), M.insert b [] m2)
...
safe :: Eq a => Map (Block a) [Block a] -> [Block a]
safe m = filter (not . flip elem unstable) (M.keys m)
    where
        unstable = fmap head $ M.elems $ M.filter ((==1) . length) m

calc :: Integral a => String -> a
calc = genericLength . safe . snd . foldr dropBlocks (M.empty, M.empty) . sortOn ((* (-1)) . getZ) . fmap parse . lines
